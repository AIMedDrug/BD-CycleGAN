...
from keras.layers import LSTM,core,concatenate,Bidirectional
...
def concat_dense_block(x, units=56, use_dropout=False, use_batch_norm=True, use_leaky_relu=False):
    y = dense_layer(x, units=units, use_batch_norm=use_batch_norm, use_leaky_relu=use_leaky_relu)
...
def residual_bilstm_block(x, units=56, use_dropout=False, use_batch_norm=True, use_leaky_relu=False):
    y = Lambda(lambda embedding: K.expand_dims(embedding,axis=-1))(x)
    y = Bidirectional(LSTM(units=56, return_sequences=False))(y)
...
def residual_lstm_block(x, units=56, use_dropout=False, use_batch_norm=True, use_leaky_relu=False):
    y = Lambda(lambda embedding: K.expand_dims(embedding,axis=-1))(x)
    y = LSTM(units=56, return_sequences=False)(y)
